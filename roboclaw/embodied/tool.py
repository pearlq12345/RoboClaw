...
class EmbodiedToolGroup(Tool):
    ...
    async def _execute_perception(self, kwargs: dict[str, Any]) -> str | list:
        ...
        if action == "scene_understand":
            ...
            if camera_alias:
                result = vlm.describe(camera_alias, question=question, model=model)
                return f"[{camera_alias}]: {result.text}"
            # No alias → describe all
            results = vlm.describe_all(question=question, model=model)
            if not results:
        ...
        if action == "object_detect":
            ...
            result = vlm.detect_objects(camera_alias, object_name, model=model)
        ...
        if action == "what_changed":
            ...
            if os.path.exists(tmp_before):
                ...
                result = vlm.what_changed(camera_alias, before_frame, after_frame, model=kwargs.get("model"))

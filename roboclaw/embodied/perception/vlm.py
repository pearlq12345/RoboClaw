...
from roboclaw.embodied.perception.camera_grabber import frame_to_base64, grab_all_frames, grab_frame
...
class VLM:
    ...
    def __init__(self, provider: Any | None = None):
        if provider is None:
            from roboclaw.providers.factory import make_provider
            provider = make_provider()
        self._provider = provider

    def _default_model(self) -> str:
        return getattr(self._provider, "default_model", "anthropic/claude-sonnet-4-5")
    ...
    def describe(
        ...
        resp = self._provider.chat(messages, model=model, max_tokens=1024)
        return SceneDescription(text=resp.content or "", raw_response=str(resp))
    ...
    def describe_all(
        ...
        for alias, frame in frames.items():
            ...
            resp = self._provider.chat(messages, model=model, max_tokens=512)
            results[alias] = SceneDescription(text=resp.content or "")
        return results
    ...
    def detect_objects(
        ...
        return self.describe(camera_alias, question, model=model)
    ...
    def what_changed(
        ...
        resp = self._provider.chat(messages, model=model, max_tokens=512)
        return SceneDescription(text=resp.content or "")

...
The built-in RoboClaw GRPO profile is a template-level contract for RLinf post-training. It records `groupSize`, `placementStrategy`, and a Hydra config path, and it requires true import preflight. Treat the current `roboclaw_vla.rl.launcher` as a stub until actor, rollout, and env worker orchestration is implemented.
